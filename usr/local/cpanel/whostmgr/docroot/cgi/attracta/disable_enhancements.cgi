...
if ($response) {
    Cpanel::ThirdParty::Attracta::FastInclude::disable();

    print JSON::Syck::Dump( { status => 1, message => 'Attracta Site Enhancements Disabled' } );
    exit;
}
else {
    print JSON::Syck::Dump(
        {
            status  => 0,
            message => 'Could not disable Attracta Site Enhancements. Check /usr/local/cpanel/logs/error_log for details'
        }
    );
    exit;
}
